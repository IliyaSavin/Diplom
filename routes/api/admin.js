  ...
  ecoConnection.on('connect', function (err) {
    request = new Request(
      `CREATE USER ${login}
                                ...
                                GRANT DELETE ON "dbo"."Favorite_Station" TO "${login}";`,
      function (err, rowCount, rows) {
        ecoConnection.close()
        if (err) {
          console.log(err)
...
router.get('/config', auth, async (req, res) => {
  ...
  connection.on('connect', function (err) {
    var all = []
    request = new Request('select * from System_Configuration;', function (
      err,
      rowCount,
      rows
    ) {
...
router.get('/activityLog/', auth, async (req, res) => {
  ...
  connection.on('connect', function (err) {
    ...
    request = new Request(requestStr + whereStr + orderStr, function (
      err,
      rowCount,
      rows
    ) {
